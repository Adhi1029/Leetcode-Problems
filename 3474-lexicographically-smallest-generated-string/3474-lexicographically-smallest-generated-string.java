class Solution {
    public String generateString(String str1, String str2) {
        ...
        for (int i = 0; i < n; ++i) if (str1.charAt(i) == 'T') for (int j = 0; j < m; ++j) {
            final int pos = i + j;
            if (ans[pos] != 0 && ans[pos] != str2.charAt(j)) return "";
            ans[pos] = str2.charAt(j);
            modifiable[pos] = false;
        }
        for (int i = 0; i < sz; ++i) if (ans[i] == 0) ans[i] = 'a';
        for (int i = 0; i < n; ++i) if (str1.charAt(i) == 'F' && match(ans, i, str2)) {
            final int modifiablePos = lastModifiablePosition(i, m, modifiable);
            if (modifiablePos == -1) return "";
            ans[modifiablePos] = 'b';
    ...
    private boolean match(char[] ans, int i, String s) {
        for (int j = 0; j < s.length(); ++j) if (ans[i + j] != s.charAt(j)) return false;
        return true;
    }
    private int lastModifiablePosition(int i, int m, boolean[] modifiable) {
        ...
        for (int j = 0; j < m; ++j) {
            final int pos = i + j;
            if (modifiable[pos]) modifiablePos = pos;
        }
