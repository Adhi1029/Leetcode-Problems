...
public:
    ...
    int getRank(const vector<vector<int>>& graph, int u, int currRank,vector<int>&& rank, vector<vector<int>>& ans) {
        if (rank[u] != NO_RANK)
        return rank[u];
        ...
        for (const int v : graph[u]) {
            ...
            continue;
            const int nextRank=getRank(graph,v,currRank+1,std::move(rank),ans);
